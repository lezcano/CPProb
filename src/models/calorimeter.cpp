...
int get_index(const std::vector<double> &edges, float value) {
    auto index = std::distance(edges.begin(),
                               std::find_if(edges.begin(), edges.end(), [value](double x) { return x > value; })) - 1;
...
double particle_calorimeter_response(
        ...
        const std::vector<std::vector<double>> &calo_segmentation,
...
std::vector<std::vector<std::vector<double>>> calo_simulation(const std::vector<std::vector<double>> &particle_data) {
    ...
    std::vector<std::vector<double>> histo_edges{bin_edges(-3, 3, NBINX),
                                                 bin_edges(-3, 3, NBINY),
    ...
    for (const auto &particle_data : particle_data) {
