...
namespace cpprob {
    ...
    public:
        ...
        {
            // are we copying between the same type?
            cpprob::detail::fxn_ptr_table<Char>* x_table =
                cpprob::detail::get_table<T>::template get<Char>();
            if (table == x_table) {
        ...
        T const& cast() const
        {
            ...
            return cpprob::detail::get_table<T>::is_small::value ?
...
// implicit casting is disabled by default for compatibility with boost::any
        ...
        bool empty() const
        {
            return table == cpprob::detail::get_table<cpprob::detail::empty>::template get<Char>();
        }
...
#endif
        // fields
        cpprob::detail::fxn_ptr_table<Char>* table;
        void* object;
    };
    ...
    typedef basic_hold_any<char> any;
    typedef basic_hold_any<wchar_t> wany;

    namespace traits
    {
        template <typename T>
        struct is_hold_any : boost::mpl::false_ {};

        template <typename Char>
        struct is_hold_any<basic_hold_any<Char> > : boost::mpl::true_ {};
    }
